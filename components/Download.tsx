...
const Download: FC<PropTypes> = ({ sportType }) => {
	...
	const downloadCsv = () => {
		for (const div of DivTypes) {
			for (const type of TeamTypes) {
				...
				const gamesClean = gamesRaw.map(elm => {
					const homeCode = schoolData.filter(elm2 => elm2.school_name === elm.homeTeam.schoolName)[0].code;
					const awayCode = schoolData.filter(elm2 => elm2.school_name === elm.awayTeam.schoolName)[0].code;
					const date = `${elm.date.getMonth() + 1}/${elm.date.getDate()}/${elm.date.getFullYear()}`;
	...
	return (
		<button onClick={downloadCsv} type="button" className="my-border my-shadow group m-2 flex h-14 w-[3rem] items-center overflow-hidden rounded-md bg-main p-2 duration-75 ease-in-out hover:w-[10rem] hover:scale-110 active:scale-90">
			...
			<p className="h-fit w-fit translate-x-[-110%] whitespace-nowrap font-semibold text-invert  duration-300 group-hover:translate-x-[30%]">Download Csv</p>
		</button>
	);
};
